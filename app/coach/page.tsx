...
import { apiCoach } from "@/lib/api-client";
import * as db from "@/lib/db";
import { computeInsights, localCoach, type Insight } from "@/lib/insights";
import { lastNDates } from "@/lib/progress";
import type { CoachRequest, CoachResult, DayLog, Profile } from "@/lib/schemas";
...
export default function CoachPage() {
  ...
  return <Coach profile={profile} targets={calc.targets} today={today} days={days} />;
}
...
function Title() {
  return (
    <header>
      ...
      <h1 className="text-2xl font-extrabold tracking-tight">What to change, from what you actually eat</h1>
    </header>
  );
}

function Coach({ profile, targets, today, days }: { profile: Profile; targets: Targets; today: string; days: DayLog[] }) {
  ...
  return (
    <div className="stagger space-y-6">
      ...
      <div className="segmented" role="tablist" aria-label="Review type" style={{ ["--i" as string]: 1 }}>
      ...
      <Review key={mode} mode={mode} profile={profile} targets={targets} today={today} days={days} insights={insights} />
...
function buildRequest(mode: Mode, profile: Profile, targets: Targets, today: string, days: DayLog[], insights: Insight[]): CoachRequest {
  const now = new Date();
...
function Review({ mode, profile, targets, today, days, insights }: { mode: Mode; profile: Profile; targets: Targets; today: string; days: DayLog[]; insights: Insight[] }) {
  const { online } = useApp();
  const [state, setState] = useState<State>({ kind: "idle" });
  const [revealed, setRevealed] = useState(false);
  const req = useMemo(() => buildRequest(mode, profile, targets, today, days, insights), [mode, profile, targets, today, days, insights]);
